...
var submitcity = function (event) {
    ...
    if (usercity) {
        ...
        var exists = storage.find(x => x === usercity)
        if (!exists) storage.push(usercity)
                                ...
                                if (response.status === 200) {
                                    ...
                                    return response.json()
                                        ...
                                        .then(function (data) {
                                            ...
                                            // Using the below code (for loop) into a seperate function
                                            //console.log(imgIcon)
                                            //cdate.textContent = todayDate;
                                            //console.log(todayDate);
                                            //console.log(today);
                                            //weatherForecast(data, today);
...
//function weatherForecast(newData, newToday) {
    function weatherForecast(newData) {
    for (var i = 7; i < newData.list.length; i += 8) {
        var day = newData.list[i]
        var fiveDate = dayjs.unix(day.dt).format("M/D/YYYY");
        //console.log(fiveDate);
        //console.log(day)
